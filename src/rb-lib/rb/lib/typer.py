...
def get_inputs_from_signature(
    ...
    # Try to get the schema from the 'endpoint' object in the command's closure.
    # This is the most reliable way for commands created by MLService.
    task_schema = None
            ...
            if schema_command:
                try:
                    ...
                    if endpoint and hasattr(endpoint, "task_schema_func"):
                        ...
                        # If we successfully parsed from the schema, we are done.
                        return result
                except (ValueError, IndexError, AttributeError):
                    # If 'endpoint' is not found or something is wrong,
                    # fall through to the generic signature parsing below.
                    pass
    # --- End of MLService-specific logic ---

    # --- Fallback generic logic for non-MLService commands ---
