...
for plugin in rescuebox_app.registered_groups:
    ...
    for command in plugin.typer_instance.registered_commands:
        ...
        desktop_api_prefix =f'/api/'
        schema_suffix =f'_schema'
        if command.name and (command.name.startswith(desktop_api_prefix) or command.name.endswith(schema_suffix)):
            ...
            if command.name.startswith(desktop_api_prefix):
                # TODO: this prefix '/api' has to change when there are multiple plugins
                cli_router.include_router(router, prefix='/api', tags=[plugin.name])
            else:
