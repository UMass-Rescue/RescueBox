...
from rb.api.routes.cli_utils import (
...
info_file_path = os.path.join(
    "src", "rb-audio-transcription", "rb_audio_transcription", "app-info.md"
)
...
@app.command("/api/app_metadata")  # ("/api/app_metadata") Electron desktop call
...
@app.command("api/routes")  # /api/routes Electron desktop call
def routes() -> Any:
    """return routes"""
    logger.info("routes called")
    route = {
        "task_schema": "/audio/task_schema",
        "run_task": "/audio/transcribe",
        "payload_schema": "/audio/payload_schema",
        "sample_payload": "/audio/facematch",
    ...
    logger.info(type(routes))
    return route_list


# Configure UI Elements in RescueBox Desktop
@app.command("/audio/task_schema")
...
def cli_inputs_parser(input_path: str) -> FileInputs:
    ...
    try:
        ...
        raise typer.Abort()
        #return FileInputs(file_inputs=BatchFileInput(files=[]))
