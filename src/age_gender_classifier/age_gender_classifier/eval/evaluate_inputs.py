...
def age_histogram(
    df: pd.DataFrame, saveFile: str = "imgs/age_distribution.png"
) -> None:
    """"""
...
def gender_counts(df: pd.DataFrame, saveFile: str = "imgs/gender_counts.png") -> None:
    ...
    sns.barplot(
        x=["Male", "Female"],
        y=[gender_counts[0], gender_counts[1]],
        palette=["gray", "orange"],
        hue=["Male", "Female"],
        width=0.5,
    )
...
def main():
    """"""
    df = read_db(
        table_name="age_gender_labeled", query="SELECT * FROM age_gender_labeled"
    )
    age_histogram(df)
    gender_counts(df)
