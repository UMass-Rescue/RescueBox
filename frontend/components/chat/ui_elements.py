...
def create_chat_header(on_show_history: Optional[Callable] = None):
    with ui.row().classes("rb-chat-toolbar-floating items-center justify-end w-full px-4 py-3 sticky top-0 z-10 gap-3"):
        models_btn = ui.button("Menu").classes(Design.BTN_PRIMARY_COMPACT).props("unelevated no-caps")
        analyze_btn = ui.button("Chat").classes(Design.BTN_PRIMARY_COMPACT).props("unelevated no-caps")
        history_btn = ui.button("History", on_click=on_show_history).classes(Design.BTN_PRIMARY_COMPACT).props("unelevated no-caps")
...
def create_chat_window() -> Any:
    ...
    container = ui.column().classes('rb-chat-messages-scroll w-full flex-1 overflow-y-auto p-6 space-y-4 bg-white')
    render_welcome_message(container)
    return container

def create_input_area(status_text_ref: Optional[object], on_send: Callable):
    input_area = ui.column().classes("rb-chat-input-area w-full flex-none bg-white border-t p-4")
    set_latest_input_area(input_area)
    with input_area:
        with ui.column().classes("rb-chat-composer-core w-full") as composer_strip:
            input_field = ui.textarea(label="Type your request").classes(Design.INPUT_MODERN).props("rows=4")
            with ui.row().classes("w-full items-center gap-3 mt-2"):
                send_button = ui.button("Send", icon="send", on_click=on_send).classes(f"{Design.BTN_PRIMARY} !text-base")
