...
async def show_history_dialog(on_conversation_select: Callable[[str], None]) -> ui.dialog:
    ...
    with ui.dialog() as dialog, ui.card().classes(Design.PANEL_SHELL_CARD_WIDE):
        ...
        with ui.column().classes(f"{Design.PANEL_SHELL_BODY} gap-3 overflow-y-auto max-h-[60vh] w-full"):
...
def show_conversation_view_dialog(conversation: Any, messages: List[Any], title: str = None):
    ...
    def _render_message_card(msg: Any) -> None:
        ...
        with ui.card().classes("w-full p-3 bg-zinc-50 border border-zinc-200 rounded-lg"):
            ...
            if isinstance(tcalls, list) and tcalls:
                extras.append(
                    ("Tool calls", json.dumps(tcalls, indent=2, default=str))
                )

            for expansion_title, body in extras:
                with ui.expansion(expansion_title).classes("w-full mt-2"):
                    ui.code(body).classes("text-xs w-full whitespace-pre-wrap break-all")
    ...
    with ui.dialog() as dialog, ui.card().classes("w-full max-w-4xl max-h-[80vh]"):
        ...
        with ui.column().classes("space-y-3 overflow-y-auto max-h-[65vh] w-full min-w-0"):
            for msg in messages:
