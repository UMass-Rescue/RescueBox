    ...
    with ui.row().classes(f'w-full {align} mb-2'):
        with ui.column().classes(f'max-w-[80%] {align}'):
            ...
            with ui.row().classes('items-center gap-2 mb-1'):
                ui.label(sender).classes('text-xs font-semibold text-gray-600')
                if hasattr(message, 'timestamp') and message.timestamp:
                    from .conversation_utils import _format_timestamp
                    ui.label(_format_timestamp(message.timestamp)).classes('text-xs text-gray-500')

            # Message content
            with ui.card().classes(f'{bg_color} {border_color} border p-3 rounded-lg'):
                if getattr(message, 'message_type', 'text') == 'text':
                    ...
                    ui.label('🔧 Tool Call').classes('font-semibold mb-2')
                    ...
                    ui.label('✅ Tool Result').classes('font-semibold mb-2')
                    ui.code(getattr(message, 'content', ''), language='json').classes('text-xs')
                elif getattr(message, 'message_type', '') == 'error':
                    # Special handling for errors
                    ui.label('❌ Error').classes('font-semibold mb-2 text-red-600')
