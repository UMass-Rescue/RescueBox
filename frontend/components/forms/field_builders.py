...
async def create_input_field(
    ...
    with ui.column().classes('gap-2 w-full min-w-0'):
        ...
        if input_type == InputType.DIRECTORY:
            create_directory_input(field_id, initial_values.get(field_id, {}), form_widgets, autofill_output_key)
        elif input_type == InputType.FILE:
            create_file_input(field_id, initial_values.get(field_id, {}), form_widgets, autofill_ufdr_mount_key)
        elif input_type == InputType.TEXTAREA:
            form_widgets[field_id] = ui.textarea(
                value=initial_values.get(field_id, {}).get('text', '') if isinstance(initial_values.get(field_id), dict) else ''
            ).classes('w-full')
...
async def create_parameter_field(
    param_schema: Union[ParameterSchema, Dict],
    form_widgets: Dict,
    initial_values: Dict
) -> None:
    ...
    with ui.column().classes('gap-2'):
        ...
        if _is_ranged_float_descriptor(param_descriptor):
            ...
            form_widgets[param_id] = ui.number(value=max(rmin, min(rmax, val)), min=rmin, max=rmax, step=0.05, format='%.2f').classes('w-full')
            ...
            form_widgets[param_id] = ui.number(value=max(rmin, min(rmax, val)), min=rmin, max=rmax, step=1, format='%d').classes('w-full')
        elif isinstance(param_descriptor, FloatParameterDescriptor):
            form_widgets[param_id] = ui.number(value=float(initial_value) if initial_value is not None else param_descriptor.default, format='%.2f').classes('w-full')
        elif isinstance(param_descriptor, IntParameterDescriptor):
            form_widgets[param_id] = ui.number(value=int(initial_value) if initial_value is not None else param_descriptor.default, format='%d').classes('w-full')
        elif isinstance(param_descriptor, EnumParameterDescriptor):
            options = [opt.label or opt.key for opt in param_descriptor.enum_vals if opt.label or opt.key]
            l2k = { (opt.label or opt.key): opt.key for opt in param_descriptor.enum_vals }
            k2l = { v: k for k, v in l2k.items() }
            default_label = k2l.get(initial_value, initial_value if initial_value in l2k else (k2l.get(param_descriptor.default) or (options[0] if options else '')))
            ...
            form_widgets[param_id] = ui.input(value=str(initial_value) if initial_value is not None else param_descriptor.default).classes('w-full')

def create_directory_input(field_id, initial_value, form_widgets, autofill_output_key=None):
    with ui.column().classes('w-full min-w-0 gap-1'):
        ...
        with ui.row().classes('w-full min-w-0 items-center gap-2 flex-nowrap'):
            dir_input = ui.input(placeholder='/path/to/directory', value=initial_value.get('path', '') if isinstance(initial_value, dict) else '').classes('flex-1 min-w-0').props('outlined dense')
...
def create_file_input(field_id, initial_value, form_widgets, autofill_mount_key=None):
    with ui.column().classes('w-full min-w-0 gap-1'):
        ...
        with ui.row().classes('w-full min-w-0 items-center gap-2 flex-nowrap'):
            file_input = ui.input(placeholder='/path/to/file', value=initial_value.get('path', '') if isinstance(initial_value, dict) else '').classes('flex-1 min-w-0').props('outlined dense')
            v_icon = ui.icon('').classes('text-zinc-400 shrink-0')
            def validate():
