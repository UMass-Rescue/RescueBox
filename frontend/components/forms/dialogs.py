...
async def show_case_notes_dialog() -> Optional[str]:
    ...
    with ui.dialog() as dialog, ui.card().classes(Design.PANEL_SHELL_CARD_MD):
        ...
        with ui.row().classes(f"{Design.PANEL_SHELL_FOOTER} justify-end flex-wrap"):
            ui.button("Cancel", on_click=lambda: dialog.submit(None)).classes(Design.BTN_MEDIUM_GRAY)
            ui.button("Submit Job", on_click=lambda: dialog.submit((textarea.value or "").strip())).classes("rb-brand-primary text-white rounded-xl px-4 py-2")
