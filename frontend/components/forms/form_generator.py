...
class FormGenerator:
    ...
    async def generate_form(self, schema, container, initial_values=None, onSubmit=None, onCancel=None, compact=False, endpoint=None):
        if isinstance(schema, dict):
            # Normalization logic
            params = schema.get('parameters')
            if isinstance(params, dict):
                schema['parameters'] = [{'key': k, 'label': v.get('label', k.replace('_', ' ').title()), 'subtitle': v.get('subtitle', ''), 'value': v.get('value', v)} for k, v in params.items()]
        ...
        with container:
            with ui.column().classes(f"w-full min-w-0 max-w-full {'p-3 space-y-2' if compact else 'p-6 space-y-4'}"):
                ui.label('Input form').classes('text-xl font-bold' if not compact else 'text-lg font-bold')
                
                if schema.inputs:
                    ui.label('Inputs').classes('font-semibold text-lg mt-4' if not compact else 'font-semibold text-base mt-2')
...
async def handle_form_submit(schema, widgets, onSubmit, initial_inputs=None, endpoint=None):
    try:
        if onSubmit is None:
        ...
        if not v_res['is_valid']:
            handle_validation_error(v_res.get('errors', {}), "Form submission validation")
            return False
...
def validate_form(schema, widgets, initial_inputs=None, endpoint=None):
    form_data = collect_form_data(schema.model_dump() if hasattr(schema, 'model_dump') else schema, widgets, initial_inputs)
    v_res = validate_form_data(form_data, schema, endpoint=endpoint)
    if not v_res['is_valid']:
