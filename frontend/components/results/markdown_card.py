...
def render_markdown_card(container: ui.element, markdown: str) -> None:
    ...
    try:
        with container:
            with ui.card().classes('bg-gradient-to-br from-indigo-50 to-zinc-50 border-2 border-indigo-200 rounded-xl shadow-lg overflow-hidden'):
                # Header
                with ui.row().classes('bg-gradient-to-r from-indigo-500 to-indigo-700 text-white p-4 items-center'):
                ...
                with ui.scroll_area().classes('h-96'):
                    with ui.column().classes('p-6'):
                        ui.markdown(markdown).classes('prose prose-zinc prose-sm max-w-none text-zinc-800 leading-relaxed')
