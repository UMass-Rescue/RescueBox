                    ...
                    with ui.row().classes('gap-2'):
                        ...
                        if on_inspect:
                            ui.button(
                                ...
                                on_click=lambda: on_inspect(model['uid']) if on_inspect else None
                        ...
                        if not is_online and on_connect:
                            ui.button(
                                '🔌 Connect',
                                on_click=lambda: on_connect(model['uid']) if on_connect else None
...
def render_model_info_card(container: ui.element, model_info: Any, model_info_dict: Dict[str, Any], server_status: str) -> None:
    ...
    try:
        with container:
            with ui.card().classes('bg-zinc-50 border border-zinc-200 p-6 sticky top-24'):
