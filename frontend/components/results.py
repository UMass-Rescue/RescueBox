...
_SERVE_TTL = 300
_IMAGE_PREVIEW_EXTS = {'.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp', '.tif', '.tiff', '.svg'}
        ...
        if ext in _IMAGE_PREVIEW_EXTS:
            with ui.dialog() as d, ui.card().classes('max-w-[95vw] w-full p-4'):
                ...
                with ui.row().classes('gap-2 mt-2'):
                    ui.button('Open folder', on_click=lambda: open_folder(os.path.dirname(path))).props('outline')
...
def create_metadata_table_columns(base_columns: List[Dict], metadata_keys: List[str]) -> List[Dict]:
    columns = base_columns.copy()
    for key in metadata_keys:
        columns.append({'name': key.lower().replace(' ', '_'), 'label': key, 'field': key.lower().replace(' ', '_'), 'align': 'left', 'sortable': True})
    return columns
...
def _pil_image_with_bbox_drawn(source: Image.Image, bbox: Tuple[int, int, int, int]) -> Image.Image:
    ...
    if m > _MAX_PREVIEW_SIDE:
        ...
        im = im.resize((max(1, int(round(nw * scale))), max(1, int(round(nh * scale)))), Image.Resampling.LANCZOS)
        nw, nh = im.size
        x1, y1, x2, y2 = int(round(x1 * scale)), int(round(y1 * scale)), int(round(x2 * scale)), int(round(y2 * scale))
    draw = ImageDraw.Draw(im)
    draw.rectangle([max(0, min(nw-1, x1)), max(0, min(nh-1, y1)), max(x1+1, min(nw, x2)), max(y1+1, min(nh, y2))], outline='#ff0000', width=4)
    return im
...
def open_image_bbox_preview_dialog(abs_path: str, bbox: Tuple[int, int, int, int], row: Dict) -> None:
    ...
    gender, age = str(row.get('gender') or '').strip(), str(row.get('age') or '').strip()
    meta = " ".join(b for b in (gender, age) if b)
    heading = f"{title} — {meta}" if title and meta else (title or meta or os.path.basename(abs_path))
    with ui.dialog() as dialog, ui.card().classes('max-w-5xl w-full'):
        ...
        with ui.row().classes('gap-2 mt-2'):
            ui.button('Open folder', icon='folder_open', on_click=lambda: open_folder(os.path.dirname(abs_path))).props('outline')
...
def create_sortable_table(container, columns, rows, row_key='id', on_row_click=None, tip_message=None, show_row_labels=False, table_extra_classes="", tip_message_classes="text-xs text-zinc-500 mt-2"):
    with container:
        tc = f"w-full min-w-0 {table_extra_classes}".strip()
        table = ui.table(columns=columns, rows=rows, row_key=row_key).classes(tc).props('flat bordered')
        if show_row_labels:
            for r in rows:
...
def open_text_markdown_modal(filename: str, body: str) -> None:
    ...
    with ui.dialog() as dialog, ui.card().classes('max-w-[92vw] w-[min(56rem,92vw)] max-h-[90vh] flex flex-col p-4 gap-3'):
...
def render_batch_directory(container, response):
    ...
    rows = [{'path': os.path.basename(d.path), 'path_full': d.path, 'title': d.title, 'subtitle': getattr(d, 'subtitle', '')} for d in dirs]
    cols = [{'name': 'path', 'label': 'Path', 'field': 'path', 'align': 'left', 'sortable': True},
...
def render_batch_file(container, response):
    ...
    if not has_metadata:
        rows = [{'path': os.path.basename(f.path), 'path_full': f.path, 'title': f.title, 'subtitle': getattr(f, 'subtitle', ''), 'type': getattr(f, 'file_type', 'FILE')} for f in files]
        cols = [{'name': 'type', 'label': 'Type', 'field': 'type', 'align': 'center', 'sortable': True},
...
def render_directory(container, response):
    try:
        ...
        with container, ui.card().classes('w-full bg-zinc-50 border border-zinc-200 p-4 rounded-xl shadow-sm'):
            ui.label('Directory Result').classes('text-xs font-bold text-[#505759] uppercase tracking-wider mb-1')
            ...
            if path and os.path.isdir(path):
                ...
                if not files:
                    ui.label('Directory is empty').classes('text-sm text-zinc-500 italic mt-4')
                else:
                    rows = [{'filename': f, 'path': os.path.join(path, f)} for f in files]
                    cols = [{'name': 'filename', 'label': 'Filename', 'field': 'filename', 'align': 'left', 'sortable': True}]
...
def render_file(container, response):
    try:
        ...
        with container, ui.card().classes('w-full bg-white border border-zinc-200 p-4 rounded-xl shadow-sm'):
            ui.label('📄 File Result').classes('text-xs font-bold text-zinc-500 uppercase tracking-wider mb-1')
            with ui.row().classes('items-center justify-between w-full'):
                ui.label(display_title).classes('text-xl font-semibold text-zinc-900')
...
_RENDERERS_MAP = {
    ...
    'text': 'render_text', 'markdown': 'render_markdown', 'batchtext': 'render_batch_text',
...
def _open_image_summary_markdown_modal(file_info: Dict[str, Any]) -> None:
    ...
    txt, name, path_full = file_info.get('content', ''), file_info.get('filename', 'Summary'), file_info.get('path', '')
    with ui.dialog() as dialog:
        ...
        with ui.card().classes('w-full h-full min-h-0 flex flex-col p-6 rounded-none shadow-2xl border-l border-zinc-200 bg-white'):
...
def render_image_summary_json(container, data):
    ...
    out_to_in = {pr['output_path']: pr['input_path'] for pr in data.get('file_pairs', []) if isinstance(pr, dict) and pr.get('output_path') and pr.get('input_path')}
    ...
    with container, ui.card().classes('w-full p-4 shadow-md bg-white'):
        ui.label(f"📸 Image Summaries ({len(file_data)} files)").classes('text-lg font-bold mb-4')
        with ui.element('div').classes('w-full rounded-lg border-2 border-[#505759] bg-white p-3 shadow-sm mb-4'):
            with ui.row().classes('items-center gap-2 mb-2'):
                ui.icon('search', size='1.5rem').classes('text-[#505759]')
                ui.label('Search Descriptions').classes('text-lg font-bold text-[#505759]')
            search_input = ui.input(placeholder='Filter by description...').classes('w-full rb-image-summary-search-field').props('clearable outlined dense')
                        ...
                        with ui.element('div').classes('grid min-w-[720px] grid-cols-[12rem_minmax(0,1fr)] gap-3 py-2 border-b border-zinc-100'):
                            ...
                            with ui.element('div').classes('grid grid-cols-[12rem_minmax(0,1fr)] gap-3 cursor-pointer hover:bg-zinc-50 p-1').on('click', lambda _e, f=fi: _open_image_summary_markdown_modal(f)):
                                ...
                                with ui.column().classes('border-l pl-2 max-h-56 overflow-y-auto'):
                                    ui.markdown(fi['content'] or '_(empty)_').classes(_MD_INLINE)

        search_input.on('update:modelValue', lambda e: render_rows([f for f in file_data if e.args.lower() in f['content_lower']] if e.args else file_data))
...
def render_batch_text(container, response):
    ...
    with container, ui.card().classes('w-full p-0 shadow-sm border rounded-xl overflow-hidden bg-white'):
        with ui.row().classes('w-full px-4 py-3 items-center gap-2 border-b border-zinc-200 bg-gradient-to-r from-zinc-50 to-white'):
            ...
            ui.label(f'{len(texts)} file(s)').classes('text-xs text-zinc-500 ml-auto tabular-nums')
        with ui.column().classes('w-full p-4'):
            with ui.scroll_area().classes('w-full h-[60vh]'):
                for i, t in enumerate(texts, 1):
                    with ui.column().classes('w-full min-w-0 gap-2 pb-4 border-b border-zinc-100 last:border-b-0 last:pb-0 mb-4'):
                        ui.label('Source').classes('text-xs font-medium text-zinc-500 uppercase tracking-wide')
                        ui.label(getattr(t, 'title', f'Item {i}') or f'Item {i}').classes('text-sm font-semibold text-zinc-900 break-all')
                        with ui.scroll_area().classes('w-full max-h-80 rounded-lg bg-zinc-50 ring-1 ring-zinc-200'):
                            ui.label(getattr(t, 'value', '') or '').classes('text-sm text-zinc-800 whitespace-pre-wrap leading-relaxed p-3 block')
...
def render_searchable_file_list(container, file_paths, title):
    ...
    with container, ui.card().classes('w-full bg-white border border-zinc-300 rounded-xl p-4 shadow-sm'):
        ui.label(f'{title} ({len(file_data)} files)').classes('text-lg font-bold text-zinc-900 mb-4')
        with ui.element('div').classes('w-full rounded-lg border-2 border-[#505759] bg-white p-3 shadow-sm mb-4'):
            with ui.row().classes('items-center gap-2 mb-2'):
                ui.icon('search', size='1.5rem').classes('text-[#505759]')
                ui.label('Search').classes('text-lg font-bold text-[#505759]')
            search_input = ui.input(placeholder='Filter by content...').classes('w-full rb-image-summary-search-field').props('clearable outlined dense')
        table_container = ui.column().classes('w-full')
        def update_table(search_term=''):
            search_lower = search_term.lower().strip()
            filtered = [f for f in file_data if search_lower in f['content_lower']] if search_lower else file_data
            table_container.clear()
            cols = [{'name': 'filename', 'label': 'Filename', 'field': 'filename', 'align': 'left', 'sortable': True},
                    {'name': 'content', 'label': 'Preview', 'field': 'content', 'align': 'left', 'sortable': True}]
            rows = [{'filename': f['filename'], 'content': f['content'][:400] + '...' if len(f['content']) > 400 else f['content'], 'path': f['path']} for f in filtered]
            def _on_click(e):
                idx = _resolve_row_idx(e, rows)
                if idx is not None:
                    row = rows[idx]
                    open_text_markdown_modal(row['filename'], Path(row['path']).read_text(encoding='utf-8', errors='replace'))
            create_sortable_table(table_container, cols, rows, row_key='filename', on_row_click=_on_click,
    ...
    with container, ui.card().classes('w-full p-0 shadow-lg border rounded-xl overflow-hidden bg-white'):
        with ui.row().classes('w-full p-4 items-center border-b border-zinc-100'):
            ...
            if title and title != 'Text Result':
                ui.label(f'• {title}').classes('ml-2 opacity-90 text-sm font-medium text-zinc-700')
        with ui.scroll_area().classes('w-full h-96'):
            with ui.column().classes('w-full p-6'):
                ui.markdown(val).classes('prose prose-sm max-w-none text-zinc-900 leading-relaxed')
...
create_directory_row_click_handler = create_bbox_preview_row_click_handler # Use same logic for now
