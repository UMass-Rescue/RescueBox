...
def _resolve_input_path(path_value: Any) -> Path:
    ...
    if not path_str: return Path(path_str)
    p = Path(path_str)
    if p.is_absolute() and p.exists(): return p
    if p.exists(): return p.resolve()
    try:
        demo_p = DEMO_FOLDERS_BASE / path_str
        if demo_p.exists(): return demo_p.resolve()
    except: pass
    return p.resolve()

def is_outputs_results_directory(path: str) -> bool:
    if not path: return False
    try: return Path(path).resolve().name.casefold() == "outputs"
    except: return Path(path).name.casefold() == "outputs"
...
def maybe_autofill_output_dir_field(form_widgets, output_field_id, valid_input_dir):
    ...
    if w is None or getattr(w, 'value', None): return
    suggested = suggested_outputs_dir_path(valid_input_dir)
    if suggested:
        try: w.set_value(suggested)
        except: w.value = suggested
...
def apply_ufdr_mount_autofill_after_inputs_built(form_widgets: Dict, ufdr_file_field_id: str, mount_folder_field_id: str):
    ...
    if not ufdr_w or not mount_w: return
    
    def on_change(e):
        if not mount_w.value:
            suggested = suggested_ufdr_mount_folder_path(e.value)
            if suggested: mount_w.set_value(suggested)
...
def maybe_autofill_ufdr_mount_name_field(form_widgets: Dict, mount_name_field_id: str, ufdr_file_path: str):
    ...
    if not w or w.value: return
    
    raw = (ufdr_file_path or "").strip()
    if not raw: return
    p = Path(raw).name
    name = p.rsplit('.', 1)[0]
    try: w.set_value(name)
    except: w.value = name

def _directory_contains_raster_image(root: Path, max_files_scanned: int = 12000) -> bool:
    try:
        resolved = root.expanduser().resolve(strict=False)
        if not resolved.is_dir(): return False
        scanned = 0
        for p in resolved.rglob("*"):
            if not p.is_file() or p.name.startswith("."): continue
            scanned += 1
            if scanned > max_files_scanned: return False
            if any(p.name.lower().endswith(s) for s in _COMMON_RASTER_IMAGE_SUFFIXES): return True
    except: pass
    return False
...
def _resolved_file_browser_folder(initial: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        rp = p.resolve()
        if rp.is_dir(): return str(rp)
        if rp.is_file(): return str(rp.parent.resolve())
    except: pass
    return None
