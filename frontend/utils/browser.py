...
from .paths import (
    _resolved_existing_directory, _resolved_file_browser_folder
)
...
if sys.platform == 'win32':
    try:
        ...
        win32api = None
        logger.warning("pywin32 is not installed. Windows-specific features are disabled.")
else:
...
def _add_windows_drives_toggle(container, on_drive_change, current_path):
    if platform.system() == 'Windows' and win32api:
        try:
            ...
            initial = current_path[0:3] if current_path and len(current_path) >= 3 and current_path[1] == ':' else drives[0]
            with container:
    ...
    def _render_directory_tree(self, path):
        ...
        try:
            ...
            if not requested_path.absolute().is_relative_to(user_root.absolute()):
                logger.error("Error is_relative_to: %s %s", requested_path.absolute(), user_root.absolute())
            ...
            with self.file_list:
                ...
                if p_obj.parent != p_obj:
                    with ui.row().classes('w-full items-center gap-3 px-3 py-2 cursor-pointer hover:bg-zinc-100 rounded-lg transition-colors').on('click', lambda: self._render_directory_tree(str(p_obj.parent))):
                        ui.icon('arrow_upward', size='sm').classes('text-zinc-500')
                        ui.label('.. (Parent Directory)').classes('text-sm font-medium text-zinc-600')

                # Subdirectories
                items = sorted(p_obj.iterdir(), key=lambda x: (not x.is_dir(), x.name.lower()))
                for item in items:
                    if item.is_dir():
                        with ui.row().classes('w-full items-center justify-between px-3 py-1 hover:bg-zinc-100 rounded-lg transition-colors group'):
                            # Left side: Navigation
                            with ui.row().classes('items-center gap-3 cursor-pointer flex-1 py-1').on('click', lambda *a, p=str(item): self._render_directory_tree(p)):
                            ...
                            ui.button('Select', on_click=lambda *a, p=str(item): (self.on_select(p), self.dialog.close())) \
                                .props('flat dense color=primary') \
                                .classes('text-xs opacity-0 group-hover:opacity-100 transition-opacity font-bold uppercase tracking-wider')
        except Exception as e:
            logger.error("Error rendering directory tree: %s", e)
            with self.file_list:
                ui.label(f"Access denied or error: {str(e)}").classes('text-xs text-red-500 p-2')

    def show(self):
        # Use PANEL_SHELL_CARD instead of WIDE to avoid clipping on smaller screens
        with ui.dialog() as self.dialog, ui.card().classes(Design.PANEL_SHELL_CARD + ' h-[80vh] max-h-[800px]'):
            # Header
            with ui.row().classes(Design.PANEL_SHELL_HEADER):
                with ui.row().classes('items-center gap-2'):
                    ui.icon('folder_open', size='md').classes('text-[#881c1c]')
                    ui.label("Select Directory").classes(Design.PANEL_SHELL_HEADER_TITLE)
                ui.button(icon='close', on_click=self.dialog.close).props('flat round').classes(Design.PANEL_SHELL_HEADER_ICON)
            
            # Body
            with ui.column().classes(Design.PANEL_SHELL_BODY + ' gap-3'):
                # Drive selector for Windows
                _add_windows_drives_toggle(ui.column().classes('w-full'), self._render_directory_tree, self.state['current_path'])
                
                # Path display
                with ui.row().classes('w-full items-center gap-2 p-2 bg-zinc-50 rounded-lg border border-zinc-200'):
                    ui.label('Location:').classes('text-xs font-bold text-zinc-500 uppercase shrink-0')
                    self.path_display = ui.label(self.state['current_path']).classes('text-sm font-mono text-zinc-700 break-all')
                
                # Directory list - use flex-1 to fill available body space
                self.file_list = ui.column().classes('w-full flex-1 overflow-y-auto border border-zinc-100 rounded-xl p-2 bg-white min-h-0')
                self._render_directory_tree(self.state['current_path'])
            
            # Footer
            with ui.row().classes(Design.PANEL_SHELL_FOOTER + ' justify-end'):
                ui.button('Cancel', on_click=self.dialog.close).classes(Design.BTN_MEDIUM_GRAY).props('outline')
                ui.button('Select This Folder', on_click=lambda: (self.on_select(self.state['current_path']), self.dialog.close())).classes(Design.BTN_PRIMARY)
...
class FileBrowser:
    ...
    def _render_file_tree(self, path):
        ...
        try:
            ...
            with self.file_list:
                # Parent directory
                if p_obj.parent != p_obj:
                    with ui.row().classes('w-full items-center gap-3 px-3 py-2 cursor-pointer hover:bg-zinc-100 rounded-lg').on('click', lambda: self._render_file_tree(str(p_obj.parent))):
                        ui.icon('arrow_upward', size='sm').classes('text-zinc-500')
                        ui.label('.. (Parent Directory)').classes('text-sm font-medium text-zinc-600')
                ...
                # Render Directories
                for item in dirs:
                    with ui.row().classes('w-full items-center gap-3 px-3 py-2 cursor-pointer hover:bg-zinc-100 rounded-lg').on('click', lambda *a, p=str(item): self._render_file_tree(p)):
                        ui.icon('folder', size='sm').classes('text-[#881c1c]')
                        ui.label(item.name).classes('text-sm font-medium text-zinc-800')
                ...
                for item in files:
                    ...
                    if self.filetypes and not any(item.name.lower().endswith(ft.lower()) for ft in self.filetypes):
                        continue
                        
                    with ui.row().classes('w-full items-center gap-3 px-3 py-2 cursor-pointer hover:bg-[#881c1c]/10 rounded-lg group').on('click', lambda *a, p=str(item): self._select_file(p)):
                        ui.icon('insert_drive_file', size='sm').classes('text-zinc-400 group-hover:text-[#881c1c]')
                        ui.label(item.name).classes('text-sm text-zinc-700 group-hover:text-zinc-900')
    ...
    def show(self):
        with ui.dialog() as self.dialog, ui.card().classes(Design.PANEL_SHELL_CARD + ' h-[80vh] max-h-[800px]'):
            ...
            # Body
            with ui.column().classes(Design.PANEL_SHELL_BODY + ' gap-3'):
                _add_windows_drives_toggle(ui.column().classes('w-full'), self._render_file_tree, self.state['current_path'])
            ...
            with ui.row().classes(Design.PANEL_SHELL_FOOTER):
                with ui.row().classes('flex-1 items-center gap-2 overflow-hidden'):
                    ui.label('Selected:').classes('text-xs font-bold text-zinc-500 uppercase shrink-0')
                    self.selection_label = ui.label('None').classes('text-sm font-medium text-[#881c1c] truncate')
                
                ui.button('Cancel', on_click=self.dialog.close).classes(Design.BTN_MEDIUM_GRAY).props('outline')
                self.confirm_btn = ui.button('Confirm Selection', on_click=lambda: (self.on_select(self.state['selected_file']), self.dialog.close())).classes(Design.BTN_PRIMARY)
                self.confirm_btn.set_visibility(False)
