...
def notify_success(message: str, duration: float = 3.0, position: str = 'top', close_button: bool = True, **kwargs):
    ...
    _safe_ui_call(ui.notify, message, type='positive', timeout=int(duration * 1000), position=position, close_button=close_button, **kwargs)

def notify_error(message: str, duration: float = 5.0, position: str = 'top', close_button: bool = True, **kwargs):
    logger.debug(f"Error notification shown: {message}")
    _safe_ui_call(ui.notify, message, type='negative', timeout=int(duration * 1000), position=position, close_button=close_button, **kwargs)

def notify_info(message: str, duration: float = 3.0, position: str = 'top', close_button: bool = True, **kwargs):
    logger.debug(f"Info notification shown: {message}")
    _safe_ui_call(ui.notify, message=message, timeout=int(duration * 1000), position=position, close_button=close_button, **kwargs)

def notify_warning(message: str, duration: float = 4.0, position: str = 'top', close_button: bool = True, **kwargs):
    logger.debug(f"Warning notification shown: {message}")
    _safe_ui_call(ui.notify, message, type='warning', timeout=int(duration * 1000), position=position, close_button=close_button, **kwargs)
...
def handle_validation_error(errors: Union[dict, list], context_str: str = "Form validation failed"):
...
def require_demo_user_session():
    from frontend.utils.storage import get_user_id_for_jobs, ensure_explicit_user_id_for_tests
