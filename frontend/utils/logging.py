...
_NOISY_WARNING_NAMES = (
    "socketio.server", "socketio", "engineio.server", "engineio", "fuse", "fuse.log-mixin",
    "ufdr_mounter.utils.ufdr_mount_unix", "frontend.pages.chatbot", "frontend.database.chat_history_db",
    "frontend.utils.nicegui_storage", "httpcore", "httpcore.http11", "httpx", "frontend.chatbot",
    "frontend.components.forms", "frontend.components.results.tool_selection_card",
    "frontend.utils.validators", "frontend.utils.file_browser", "nicegui",
)

_CHATBOT_FORMS_WARNING_NAMES = (
    "frontend.pages.chatbot.chatbot_forms", "frontend.pages.chatbot.chatbot",
    "frontend.pages.chatbot.state.state_manager", "frontend.pages.chatbot.utils.form_validator",
    "frontend.components.forms.form_generator", "frontend.components.forms.form_handlers",
    "frontend.components.forms.builders.input_field_builder", "frontend.components.forms.case_notes_dialog",
)
...
def configure_logging_with_context(log_file_path=None, log_level='DEBUG'):
    ...
    root.handlers.clear()
    formatter = logging.Formatter('%(asctime)s | %(levelname)-8s%(context)s | %(name)s | %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
async def generate_audit_trail_for_job(job_id: str) -> Dict[str, Any]:
    ...
    audit = {
        ...
        'job': job_dict, 'related_chat_messages': related_messages
    }
    try:
...
def format_audit_trail_markdown(audit_trail: Dict[str, Any]) -> str:
    lines = ["# RescueBox Audit Trail", "", f"**Generated:** {audit_trail.get('generated_at')}", "", "## Job Information", ""]
