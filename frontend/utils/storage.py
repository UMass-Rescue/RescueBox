...
def get_user_id() -> Optional[str]:
    try:
        ...
        if _runs_under_pytest(): return 'test-user-1'
        return None

def get_explicit_user_id() -> Optional[str]:
    try: return app.storage.user.get("explicit_job_user_id")
    except:
        if _runs_under_pytest(): return _test_fallback_storage.get("explicit_job_user_id")
        return None

def set_explicit_user_id(value: str):
    v = value.strip()
    try: app.storage.user["explicit_job_user_id"] = v
    except: pass
    if _runs_under_pytest(): _test_fallback_storage["explicit_job_user_id"] = v

def clear_explicit_user_id():
    uid = get_explicit_user_id()
    if uid:
        release_explicit_user_id_claim(uid)
    try: app.storage.user.pop("explicit_job_user_id", None)
    except: pass
    if _runs_under_pytest(): _test_fallback_storage.pop("explicit_job_user_id", None)
...
def try_claim_explicit_user_id(uid: str) -> str:
    ...
    if _runs_under_pytest():
        registry = _test_fallback_storage.get('user_id_registry', {})
        if uid in registry: return "taken"
        registry[uid] = True
        _test_fallback_storage['user_id_registry'] = registry
...
def set_user_preference(key: str, value: Any):
    ...
    prefs[key] = value
    try: app.storage.user['preferences'] = prefs
    except: pass
    if _runs_under_pytest(): _test_fallback_storage['preferences'] = prefs
...
def set_current_conversation_id(conversation_id: Optional[str]):
    try:
        if conversation_id: app.storage.user['current_conversation_id'] = conversation_id
        else: app.storage.user.pop('current_conversation_id', None)
    except: pass
    if _runs_under_pytest():
        if conversation_id: _test_fallback_storage['current_conversation_id'] = conversation_id
        else: _test_fallback_storage.pop('current_conversation_id', None)
...
def set_draft_message(message: str):
    try:
        if message: app.storage.client['draft_message'] = message
        else: app.storage.client.pop('draft_message', None)
    except: pass
    if _runs_under_pytest():
        if message: _test_fallback_storage['draft_message'] = message
        else: _test_fallback_storage.pop('draft_message', None)
...
def set_conversation_to_load(conversation_id, conversation_data, messages):
    ...
    try: app.storage.user['conversation_to_load'] = data
    except: pass
    if _runs_under_pytest(): _test_fallback_storage['conversation_to_load'] = data

def get_conversation_to_load():
    try:
        data = app.storage.user.get('conversation_to_load')
        if data: app.storage.user.pop('conversation_to_load', None)
        return data
    except:
        data = _test_fallback_storage.get('conversation_to_load')
        if data: _test_fallback_storage.pop('conversation_to_load', None)
        return data

def clear_conversation_to_load():
    try: app.storage.user.pop('conversation_to_load', None)
    except: pass
    if _runs_under_pytest(): _test_fallback_storage.pop('conversation_to_load', None)
...
def set_form_draft(endpoint: str, arguments: dict = None):
    ...
    if (not endpoint and not arguments) or (endpoint == "" and (arguments is None or arguments == {})):
        draft = None
    elif isinstance(endpoint, dict) and arguments is None:
        draft = endpoint
    else:
        draft = {'endpoint': endpoint, 'arguments': arguments}
    try: app.storage.user['form_draft'] = draft
    except: pass
    if _runs_under_pytest(): _test_fallback_storage['form_draft'] = draft

def clear_form_draft():
    try: app.storage.user.pop('form_draft', None)
    except: pass
    if _runs_under_pytest(): _test_fallback_storage.pop('form_draft', None)
...
def set_user_preferences(prefs: Dict[str, Any]):
    ...
    try: app.storage.user['preferences'] = current
    except: pass
    if _runs_under_pytest(): _test_fallback_storage['preferences'] = current

def reset_user_preferences():
    try: app.storage.user['preferences'] = DEFAULT_PREFERENCES
    except: pass
    if _runs_under_pytest(): _test_fallback_storage['preferences'] = DEFAULT_PREFERENCES
