"""Job record helpers (no UI). Kept under ``database`` so ``job_db`` can import without loading ``components``."""
...
def coerce_task_schema_and_request_body(task_schema_dict, request_body_dict):
    """Build ``TaskSchema`` and ``RequestBody`` from dict snapshots or pass through instances."""
    task_schema = (
    ...
    return task_schema, request_body
