...
class TestNotificationsUI:
    ...
    async def test_notify_success_displays(self, user: User):
        ...
        with patch('nicegui.ui.notify', side_effect=mock_notify):
            @ui.page('/test')
            async def test_page():
                ui.button('Trigger Success', on_click=lambda: notify_success("Test success message"))
    ...
    async def test_notify_error_displays(self, user: User):
        ...
        with patch('nicegui.ui.notify', side_effect=mock_notify):
            @ui.page('/test')
            async def test_page():
                ui.button('Trigger Error', on_click=lambda: notify_error("Test error message"))
    ...
    async def test_notify_warning_displays(self, user: User):
        ...
        with patch('nicegui.ui.notify', side_effect=mock_notify):
            @ui.page('/test')
            async def test_page():
                ui.button('Trigger Warning', on_click=lambda: notify_warning("Warning message"))
