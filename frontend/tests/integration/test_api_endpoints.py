...
@pytest_asyncio.fixture
async def available_models(api_client: httpx.AsyncClient) -> AsyncGenerator[List[Dict[str, Any]], None]:
    """
...
class TestModelsEndpoints:
    ...
    async def test_get_models_list(self, api_client: httpx.AsyncClient):
        ...
        assert isinstance(models, list), f"Expected list (or dict values), got {type(models)}"
        ...
        if models:
            model = models[0]
            required_fields = ['uid', 'name', 'plugin_name', 'version', 'author', 'info', 'gpu']
    ...
    @pytest.mark.asyncio
    async def test_get_model_by_uid(self, api_client: httpx.AsyncClient, available_models: List[Dict]):
        """
        ...
        assert model['uid'] == model_uid, f"UID mismatch: expected {model_uid}, got {model['uid']}"
    ...
    @pytest.mark.asyncio
    async def test_get_model_info_endpoint(self, api_client: httpx.AsyncClient, available_models: List[Dict]):
        """
        ...
        assert model_info['uid'] == direct_model['uid'], "Info endpoint should match direct endpoint"
        assert model_info['name'] == direct_model['name'], "Info endpoint should match direct endpoint"
...
class TestServersEndpoints:
    ...
    async def test_get_servers_list(self, api_client: httpx.AsyncClient):
        ...
        if servers:
            ...
            logger.info(f"First server: {server.get('modelUid')} at {server.get('serverAddress')}:{server.get('serverPort')}")
    
    @pytest.mark.asyncio
    async def test_get_server_status(self, api_client: httpx.AsyncClient, available_models: List[Dict]):
        """
        ...
        response = await api_client.get(f'/api/servers/{model_uid}/status', timeout=10.0)
        response.raise_for_status()
...
class TestModelsEndpointsIntegration:
    ...
    @pytest.mark.asyncio
    async def test_model_details_flow(self, api_client: httpx.AsyncClient, available_models: List[Dict]):
        """
...
class TestEndpointErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_models_endpoint_handles_missing_metadata(self, api_client: httpx.AsyncClient):
        """
    ...
    @pytest.mark.asyncio
    async def test_server_status_timeout(self, api_client: httpx.AsyncClient, available_models: List[Dict]):
        """
