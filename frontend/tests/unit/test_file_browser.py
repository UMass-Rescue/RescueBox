...
class TestFileBrowser:
    ...
    async def test_browse_directory_dialog(self, user: User):
        ...
        @ui.page('/test')
        def test_page():
            ui.button(BROWSE_BUTTON_TEXT, on_click=lambda: browse_directory(on_select, str(Path.cwd())))
    ...
    async def test_browse_file_dialog(self, user: User):
        ...
        @ui.page('/test')
        def test_page():
            ui.button(BROWSE_FILE_BUTTON_TEXT, on_click=lambda: browse_file(on_select, str(Path.cwd())))
    ...
    async def test_browse_directory_simple_updates_input(self, user: User):
        ...
        def test_page():
            ...
            ui.button('Browse', on_click=lambda: browse_directory_simple(input_field, str(Path.cwd())))
    ...
    async def test_browse_directory_windows_paths(self, user: User):
        ...
        @ui.page('/test')
        def test_page():
            ui.button(BROWSE_WINDOWS_BUTTON_TEXT, on_click=lambda: browse_directory(on_select, WINDOWS_PATH))
    ...
    async def test_browse_file_windows_paths(self, user: User):
        ...
        @ui.page('/test')
        def test_page():
            ui.button('Browse File Windows', on_click=lambda: browse_file(on_select, windows_path))
        ...
        def test_page():
            def test_validation():
                ...
                try:
                    ...
                    assert dir_input.path.exists() or True  # Allow non-existent for testing
    ...
    async def test_browse_directory_with_permission_error(self, user: User):
        ...
        restricted_path = '/root' if platform.system() != 'Windows' else 'C:\\System Volume Information'
    ...
    async def test_browse_file_with_filetypes_filter(self, user: User):
        ...
        def test_page():
            ui.button(
                ...
                on_click=lambda: browse_file(on_select, str(Path.cwd()), filetypes=IMAGE_FILETYPES)
            )
                ...
                def test_drives():
                    if platform.system() == 'Windows':
                        try:
                            ...
                            ui.label(f'Found {len(drives)} drives: {", ".join(drives)}').classes('text-green-600')
                        except ImportError:
    ...
    async def test_browse_directory_unix_paths(self, user: User):
        ...
        @ui.page('/test')
        def test_page():
            ui.button('Browse Unix', on_click=lambda: browse_directory(on_select, unix_path))
