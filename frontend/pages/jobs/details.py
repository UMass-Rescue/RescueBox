...
async def job_details_page_route(job_id: str):
    ...
    with ui.column().classes('w-full items-stretch px-4 sm:px-8 py-8'):
        create_breadcrumbs([{'label': 'Jobs', 'link': '/jobs'}, {'label': f'Job {job_id}'}])
        ...
        with tab_panels:
            with ui.tab_panel('Outputs'):
                try:
                    ...
                    await render_job_outputs_card(ui.column().classes('w-full'), api_c, job)
                except Exception as e:
            ...
            with ui.tab_panel('Details'):
                try:
                    ...
                    await render_job_details_panel(ui.column().classes('w-full'), api_c, jf)
                except Exception as e:
