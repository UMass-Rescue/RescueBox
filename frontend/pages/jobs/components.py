...
async def create_audit_trail_button(job_id: str):
    ...
    return ui.button('📋 Export Audit Trail', on_click=export_audit).classes('rb-brand-primary text-white rounded-xl')

def render_job_action_buttons(job_fields: Dict[str, Any]):
    model_uid = job_fields.get('modelUid')
    with ui.row().classes('gap-2'):
        if model_uid:
            ui.button('Model Doc', on_click=lambda: ui.navigate.to(f'/models/{model_uid}/details')).classes('rb-brand-primary text-white')
            ui.button('Run Model', on_click=lambda: ui.navigate.to(f'/models/{model_uid}/run')).classes('rb-brand-primary text-white rounded-xl')

def render_compact_inputs_summary(task_schema, request_body):
    try:
        from frontend.components.jobs import render_compact_inputs_summary as _render_compact
...
def render_readonly_form(task_schema, request_body):
    try:
        ...
        _render_readonly(ui.column().classes("w-full min-w-0"), task_schema, request_body)
    except Exception as e:
