...
async def fetch_task_schema(api_client, http_client, config, endpoint: str):
    ...
    if not isinstance(httpx.Client, type):
        ...
        if status_val is not None and status_val >= 400:
            if status_val == 404:
                raise httpx.HTTPStatusError("Endpoint not found", request=None, response=response)
...
async def post_job(api_client, http_client, config, api_endpoint: str, request_dict: Dict[str, Any]):
        ...
        try:
            ...
            if status_val is not None and status_val >= 400:
                ...
                if status_val == 422:
                    ...
                    raise httpx.HTTPStatusError(f"HTTP 422 Unprocessable Entity: {details}", request=None, response=response)
                last_exc = httpx.HTTPStatusError(f"HTTP {status_val}", request=None, response=response)
    ...
    if last_exc:
        raise last_exc
    raise httpx.HTTPStatusError("Unknown error submitting job", request=None, response=response)
