...
def run_audio_plugin_get_text(path: Path) -> str:
    file_path = run_audio_plugin(path)
...
@shared_task
def run_audio_plugin(path: Path) -> Path:
    try:
        ...
        if (
            isinstance(transcribe_out, BatchTextResponse)
            and len(transcribe_out.texts) > 0
        ):
...
@shared_task
def run_text_summarization_plugin(inpath: Path, outpath: Path) -> Path:
    try:
        text_in_path = inpath
        if inpath.exists():
            text_in_path = inpath.resolve()
        text_out_path = (
            Path.cwd() / ".." / "src" / "text-summary" / "audio_summary" / "output"
        )
        if outpath.exists():
